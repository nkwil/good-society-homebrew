...
**Currently in:** Session B-5 complete — moving to B-6
...
- Session B-6b (in progress):
  ...
  - B-6b/2: Reputation Phase Wizard — pending Natalie test of B-6b/1
  - B-6b/3–6: Dashboard tags, Actions modal — pending

**Next:**
- Session B-6 — robustness + retrospective: Edit Conflict Warning, Backup Export, Token Frame (per docs/design/21, 25, 27) — deferred; B-6a (Session Log) and B-6b (Reputation Batch) are the active work
- Session B-7 — Stub sweep + Phase 2/4 mechanics wiring: convert all "coming in a future session" stubs into real wired actions. Major sheet inner-conflict grid `#toggleBox`, Connection + NPC `#switchPersona` (route to `helpers/persona-swap.js` Option-B pipeline), Family `linkMajor`, Connection impressions add/edit + public tags + persona add, NPC `promoteToConnection` (actor-type conversion preserving bio/personas/scene info), NPC `addPublicTag`/`addPersona`, drag-drop reputation tag onto Major sheet (`_onDrop` → polarity routing → existing `checkThresholdAndPrompt`), wire `npc-quick-create.js` entry point if missing. Per Phase 2/4 in PLAN.md.
- Session B-8 — Magic/Skill cast pipeline: Cast button fires Sequencer effect with `vfxKey` + `soundUrl` from item; graceful degrade to chat message if Sequencer/JB2A modules absent (per CLAUDE.md §3 module-dependency rule); honors `homebrewMagicEnabled` setting; magic-visibility "reveal?" confirm if hidden. Persona-swap-on-cast path stays as-is (already works for Alter Self pattern in `helpers/persona-swap.js`). Per `docs/design/12-item-sheets.md` §Cast pipeline.
- Session B-9 — Letter composer / Epistolary UI: full ApplicationV2 framed window per `docs/design/05-epistolary-ui.md`. Two-zone layout (house chrome left, sender-themed preview right), recipient selection, seal-color picker, themed live preview, send pipeline calling existing `postLetterCard()` helper, optional journal archive. Cycle-phase aware (highlights as primary tool during epistolary phase but available always). Decide mount point during build (dock footer vs sidebar tools).
...
- ⚠ Chat card templates (`templates/chat-cards/*.hbs`) render via `ChatMessage.create → innerHTML`, NOT through ApplicationV2 PARTS, so multiple top-level elements are fine there. BUT: when the letter composer in B-9 renders `letter.hbs` through its own ApplicationV2, that template will need a single-root wrapper. Same fix as `monologue-editor.hbs` at that point.
