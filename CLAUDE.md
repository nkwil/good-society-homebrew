...
- ❌ Don't write ApplicationV2 PARTS templates with multiple top-level elements. Foundry's `_renderHTML` throws `Template part "X" must render a single HTML element` if the template resolves to more than one root node — this includes sibling tags, bare text nodes, and Handlebars comments before the opening tag. Always wrap everything in one root `<div>` or `<form>`. Note: `{{#if}}` and `{{#each}}` at the top level also count as potentially multiple nodes. This constraint applies only to PARTS templates (ApplicationV2 sheets and modals); ChatMessage content rendered via `ChatMessage.create` has no such restriction.
